...
print("theoretical max particles 4(L/a)^3 = ", int((L/a)**3 * 4))
...
def simulate(algorithm, rescaling_bool, pressure_bool, error_bool, pair_correlation_bool):
    ...
    init_r = atomic_distances(init_pos, output = 0)
    ...
    final_vector_corr = pair_correlation(init_r, dr)
    ...
    for i in range(number_of_steps):
        ...
        next_step_r = atomic_distances(next_step_position, output = 0)
        ...
        if pair_correlation_bool:   # will add condition that it only computes this when i>n_0 or something
            final_vector_corr += pair_correlation(next_step_r, dr)
    ...
    return final_vector_energy, final_vector_kin, final_vector_pot, final_vector_press, final_vector_corr

def plot_energy(potential, kinetic, total):
    print("")
