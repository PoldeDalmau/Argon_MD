...
def atomic_distances_old(pos, output): # output = 0 gives the relative distances, output = 1 gives the relative positions
...
def atomic_distances_less_old(pos, output):
    rel_pos = pos[:, None, :] - pos[None, :, :]                 # returns one matrix for each particle. Relative distances Lithin the box
...
def atomic_distances(pos, output): # output = 0 gives the relative distances, output = 1 gives the relative positions
    ...
    r = R + zeros                           # same matrix as R but with np.inf instead of 0 on the main diagonal, we need this so that we don't devide by 0 in the functions                                               lj_force, pot_en and pressure
...
def lj_force(rel_pos, r):
    ...
    -------
    np.ndarray
        The net force acting on each particle due to all other particles.
    """
    ...
    F = np.zeros((n,n,3))
    F = -24*rel_pos*((2*(1/R)**12)-((1/R)**6)) # n matricies in the form nx3, where the i-th matrix containes the force on the i-th particle due to all the other particles
    F_matrix = np.sum(F, axis=0)               # nx3 matrix, where the i-th row (F_ix F_iy F_iz) is the force on the i-th particle due to all the other particles
    return F_matrix 
...
def simulate(algorithm, T, rescaling_bool, pressure_bool, error_bool, pair_correlation_bool):
    ...
    for i in range(number_of_steps):
        ...
        if pressure_bool == True:
            #T_measure = 2*kin_en(final_matrix_vel[:,-3:])/(3*(n-1)*1)
            final_vector_tba = np.concatenate((final_vector_tba, np.array([pressure(rel_dist)])), axis = 0, out = None)
            final_vector_press = np.concatenate((final_vector_press, np.array([np.sum([rho*1/T , -(rho/(3*n))*final_vector_tba[i]])])))
        ...
        if rescaling_bool ==True:
            ...
            if i%window == 0 and rescale_flag == True:
                j = j+1
                l = np.sqrt((3*(n-1)*T)/(2*np.average(final_vector_kin[-window:])*1))
    ...
    if pressure_bool == True:
        ...
        Press = np.sum([1 , -(T/1*(3*n))*(1/(number_of_steps-n_0))*np.sum(final_vector_tba[-n_0:])])
